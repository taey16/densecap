...
function DataLoader:__init(opt)
  ...
  --[[
  self.mean = torch.FloatTensor{0.4853717905167, 0.45622173301884, 0.4061366788954}
  self.std  = torch.FloatTensor{0.22682182875849, 0.22206057852892, 0.22145828935297}
  --]]
...
function DataLoader:getBatch(opt)
  ...
  assert(split == 0 or split == 1 or split == 2, 
  ...
  -- fetch the image
  local img = self.h5_file:read('/images'):partial(
    {ix,ix},{1,self.num_channels}, {1,self.max_image_size},{1,self.max_image_size})
  ...
  img = img:view(1, img:size(1), img:size(2), img:size(3)) -- batch the image
  for c=1,3 do
    img[{{},{c},{},{}}]:add(-self.mean[c])
    img[{{},{c},{},{}}]:div(-self.std[c])
  end
  --]]
