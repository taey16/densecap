...
function DenseCapModel:__init(opt)
  -- only require loadcaffe when initializing model
  ...
  --local cnn = net_utils.load_inception(opt.cnn_name, opt.backend, opt.path_offset)
  ...
  if opt.cnn_name == 'vgg-16' then
    --[[
    conv_start1, conv_end1 = 1, 6--1, 10 -- these will not be finetuned for efficiency
    conv_start2, conv_end2 = 8, 15--11, 30 -- these will be finetuned possibly
    recog_start, recog_end = 17, 23--32, 38 -- FC layers
    opt.input_dim = 512
...
-- add script for feature extraction
