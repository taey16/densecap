...
function DenseCaptioningEvaluator:addResult(logprobs, boxes, text, target_boxes, target_text)
  ...
  logprobs = logprobs[{ {}, 1 }]:float() -- grab the positives class (1)
  ...
  table.insert(self.all_logprobs, Y:float()) -- inserting the sorted logprobs as float
end
  ...
  for foo, min_overlap in pairs(min_overlaps) do
    for foo2, min_score in pairs(min_scores) do
      ...
      for t=0,1,0.01 do
        ...
        local mask = torch.ge(rec, t):float()
        --local prec_masked = torch.cmul(prec:double(), mask)
        local prec_masked = torch.cmul(prec:float(), mask)
